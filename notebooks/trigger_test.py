...
if test_trigger_sf == 2:
    ...
    # print(combs["0"].deltaphi(combs["1"]))
    dphi = abs(combs["0"].phi - combs["1"].phi) * 180.0 / np.pi
    dphi = ak.where(dphi > 180, 360 - dphi, dphi)
    print(dphi < 80.0)
    EMTFbug_veto = ~ak.fill_none(ak.any(dphi < 80.0, axis=1), False)
    print(EMTFbug_veto)
    ...
    def dPhi(phi1, phi2):
        ...
        PI = 3.14159265359
        dphi = abs(phi1 - phi2) * 180.0 / PI
        if dphi > 180:
            dphi = 360 - dphi
        return dphi


    def get_EMTFbug_veto(pdgId_v, pt_v, eta_v, phi_v):
        temp_phi = []
        temp_eta = []
        for i in range(len(pt_v)):
            if abs(pdgId_v[i]) == 13 and pt_v[i] >= 10.0 and abs(eta_v[i]) >= 1.24:
                temp_eta.append(eta_v[i])
                temp_phi.append(phi_v[i])
        if len(temp_eta) > 1:
            for i in range(len(temp_eta)):
                for j in range(len(temp_eta)):
                    if i == j and temp_eta[i] * temp_eta[j] > 0:
                        continue
                    if dPhi(temp_phi[i], temp_phi[j]) < 80.0:
                        return 0
        return 1


    vetos = []
    for i in range(len(EMTFbug_veto)):
        if leptons[i] is not None:
            vetos.append(
                bool(
                    get_EMTFbug_veto(
                        leptons[i].pdgId, leptons[i].pt, leptons[i].eta, leptons[i].phi
                    )
                )
            )
        else:
    ...
    vetos = ak.Array(vetos)
    print(ak.all(EMTFbug_veto[: len(vetos)] == vetos))
